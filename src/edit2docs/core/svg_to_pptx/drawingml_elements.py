...
def _resolve_external_image(svg_dir: Path, href: str) -> Path:
    """Resolve an external image href relative to the SVG's directory.

    Tries the SVG's own directory first, then its parent (the project
    root for svg_output/ layouts). URL-encoded hrefs are decoded before
    probing the filesystem. Raises ``FileNotFoundError`` if none exist.
    """
...
def convert_rect(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    w = ctx_w(svg_length_x(elem.get('width'), ctx), ctx)
    h = ctx_h(svg_length_y(elem.get('height'), ctx), ctx)
...
def convert_line(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    """
    x1 = ctx_x(svg_length_x(elem.get('x1'), ctx), ctx)
    y1 = ctx_y(svg_length_y(elem.get('y1'), ctx), ctx)
    x2 = ctx_x(svg_length_x(elem.get('x2'), ctx), ctx)
    y2 = ctx_y(svg_length_y(elem.get('y2'), ctx), ctx)
...
def _build_run_xml(
    ...
    # hundredths-of-a-point. Enforce a 12pt floor: production decks
    # routinely emitted half of all text runs at 9-11pt (sub-12px SVG
    ...
    # rounded to **one decimal place of pt** (the nearest 10 hundredths). No
    # 0.5pt / integer snapping — whatever the px works out to is the size,
    # e.g. 18px -> 13.5pt, 24px -> 18.0pt, 42px -> 31.5pt. The rounding is
    # applied within the clamp above (floor/ceiling are 0.1pt-grid values).
    raw_sz = int(round(fs_px * FONT_PX_TO_HUNDREDTHS_PT / 10.0)) * 10
    ...
    return f'''<a:r>
<a:rPr lang="{run_lang}" sz="{sz}"{b_attr}{i_attr}{u_attr}{strike_attr}{spc_attr} dirty="0">
{outline_xml}
{fill_xml}
{effect_xml}
...
def convert_ellipse(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    cx_ = ctx_x(svg_length_x(elem.get('cx'), ctx), ctx)
    cy_ = ctx_y(svg_length_y(elem.get('cy'), ctx), ctx)
    rx = svg_length_x(elem.get('rx'), ctx) * ctx.scale_x
    ry = svg_length_y(elem.get('ry'), ctx) * ctx.scale_y
